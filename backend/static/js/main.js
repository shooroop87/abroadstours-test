...
// Принудительно скрыть прелоадер через 2 секунды
setTimeout(() => {
  if (!preloaderHidden) {
    console.log('Принудительное скрытие прелоадера через 2 секунды');
    hidePreloader();
  }
}, 2000);

function hidePreloader() {
  if (preloaderHidden) return; // Предотвращаем повторное выполнение
...
function initComponents() {
  ...
  // Добавляем небольшую задержку для pinOnScroll
  setTimeout(() => {
    pinOnScroll()
  }, 100)
}
