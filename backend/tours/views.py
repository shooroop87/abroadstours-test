...
class TourListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['categories'] = TourCategory.objects.filter(
            is_active=True
        ).order_by('sort_order')
        
        # SEO
        context['page_title'] = 'All Tours - Abroads Tours'
        context['page_description'] = 'Discover our amazing tours. Small group experiences from Milan to Lake Como, wine regions, and more.'
...
class TourCategoryView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        category_name = self.category.safe_translation_getter('name', any_language=True)
        category_description = self.category.safe_translation_getter('description', any_language=True)
        
        # SEO
        context['page_title'] = f"{category_name} - Tours"
        context['page_description'] = category_description or f"Discover amazing {category_name.lower()} with Abroads Tours"
...
class TourDetailView(DetailView):
    ...
    def get_object(self, queryset=None):
        ...
        try:
            ...
            logger.info(f"✅ Тур найден: ID={obj.id}, Title='{obj.safe_translation_getter('title', any_language=True)}'")
    ...
    def get_context_data(self, **kwargs):
        ...
        # SEO
        context['page_title'] = tour.safe_translation_getter('meta_title', any_language=True) or tour.safe_translation_getter('title', any_language=True)
        context['page_description'] = tour.safe_translation_getter('meta_description', any_language=True) or tour.safe_translation_getter('short_description', any_language=True)
        ...
        context['gallery_images'] = tour.images.filter(is_featured=True).order_by('sort_order')[:4]
...
def get_similar_tours(request, tour_id):
    ...
    try:
        ...
        for similar_tour in similar_tours:
            data.append({
                'id': similar_tour.id,
                'title': similar_tour.safe_translation_getter('title', any_language=True),
                'short_description': similar_tour.safe_translation_getter('short_description', any_language=True),
                'price': float(similar_tour.price_adult),
                'duration': similar_tour.get_duration_display(),
                'rating': float(similar_tour.rating),
                'featured_image': similar_tour.featured_image.url if similar_tour.featured_image else None,
                'url': similar_tour.get_absolute_url(),
                'category': similar_tour.category.safe_translation_getter('name', any_language=True) if similar_tour.category else None,
            })
